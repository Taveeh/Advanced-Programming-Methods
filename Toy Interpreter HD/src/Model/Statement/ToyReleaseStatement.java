...
public class ToyReleaseStatement implements IStatement {
    String id;

    public ToyReleaseStatement(String id) {
        this.id = id;
    }
        ...
        if (state.getSymbolTable().isVariableDefined(id)) {
            ...
            if (foundIndexValue.getType().equals(new IntegerType())) {
                ...
                if (state.getToySemaphoreTable().exists(foundIndex)) {
                    ...
                    triplet.second.remove(state.getId());
                } else {
    ...
    public IStatement createCopy() {
        return new ToyReleaseStatement(id);
    }
